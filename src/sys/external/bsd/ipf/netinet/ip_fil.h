/*	ip_fil.h,v 1.5 2013/06/29 21:06:57 rmind Exp	*/
...
typedef	int		(*ipfuncinit_t)(struct ipf_main_softc_s *,
    struct frentry *);
...
struct ipftuneable;
typedef	int (* ipftunefunc_t)(struct ipf_main_softc_s *, struct ipftuneable *,
    ipftuneval_t *);
...
# ifdef MENTAT
extern	int	ipf_check(void *, struct ip *, int, void *, int, void *,
			       mblk_t **);
#  if SOLARIS
...
#   else
extern	int	ipfioctl(dev_t, int, int *, int, cred_t *, int *);
#   endif
#  endif
#  ifdef __hpux
extern	int	ipfioctl(dev_t, int, caddr_t, int);
extern	int	ipf_select(dev_t, int);
#  endif
...
#  ifdef	__sgi
#   include <sys/cred.h>
extern	int	ipfioctl(dev_t, int, caddr_t, int, cred_t *, int *);
...
#      else
extern	int	ipfioctl(dev_t, u_long, caddr_t, int, struct thread *);
...
extern	int	ipfioctl(dev_t, u_long, caddr_t, int, struct lwp *);
#       else
extern	int	ipfioctl(dev_t, u_long, caddr_t, int, struct proc *);
#       endif
#      endif
#     endif /* __FreeBSD_version >= 500024 */
#    else
extern	int	ipfioctl(dev_t, int, caddr_t, int, struct proc *);
#    endif
#   else
#    ifdef linux
extern	int	ipfioctl(struct inode *, struct file *, u_int, u_long);
#    else
extern	int	ipfioctl(dev_t, int, caddr_t, int);
#    endif
...
#endif
extern	int	frrequest(ipf_main_softc_t *, int, ioctlcmd_t, void *,
			       int, int);
...
extern	int	ipf_ipf_ioctl(ipf_main_softc_t *, void *, ioctlcmd_t,
				   int, int, void *);
extern	int	ipf_ipftune(ipf_main_softc_t *, ioctlcmd_t, void *);
extern	int	ipf_matcharray_load(ipf_main_softc_t *, void *,
					 ipfobj_t *, int **);
...
extern	int	ppsratecheck(struct timeval *, int *, int);
#endif
...
#ifdef	USE_INET6
extern	void		ipf_inet6_mask_add(int, i6addr_t *, ipf_v6_masktab_t *);
extern	void		ipf_inet6_mask_del(int, i6addr_t *, ipf_v6_masktab_t *);
#endif
...
extern	u_short		ipf_nextipid(fr_info_t *);
...
extern	int		ipf_zerostats(ipf_main_softc_t *, void *);
